...
function Swipe(container, options) {
  ...
  var stopDirectMark = '';//立即阻止某个方向的滑动
    ...
    move: function(event) {
      ...
      if (!isScrolling) {
        ...
        if (options.continuous) { // we don't add resistance at the end
          if(stopDirectMark === 'left' && delta.x>0){
            responMoveDelta.x = delta.x/(2*Math.atan(delta.x));
          }else if(stopDirectMark === 'right' && delta.x<0){
            responMoveDelta.x = -delta.x/(2*Math.atan(delta.x));
          }else{
            responMoveDelta.x = delta.x; 
          }
          
          translate(circle(index-1), responMoveDelta.x + slidePos[circle(index-1)], 0);
          translate(index, responMoveDelta.x + slidePos[index], 0);
          translate(circle(index+1), responMoveDelta.x + slidePos[circle(index+1)], 0);
      ...
      var isValidSlide;
      if((stopDirectMark === 'left' && delta.x >0) || (stopDirectMark === 'right' && delta.x <0)){
        isValidSlide = false;
      }else{
        isValidSlide =
  ...
  return {
    ...
    stopDirect : function(d){
      if(d === '' || d === undefined || d === null || d===false){
        stopDirectMark = '';
      }else{
        stopDirectMark = d;
      }
...
(function(){
	window.$MUI = {
		loading : function(type,$dom){
			var loadHtml, $loadDom;
			$loadDom = $dom.find('.mui-ball-loading');
			if($loadDom.length === 0){
				loadHtml = ['<div class="mui-ball-loading">',
			        '<span class="mui-blue-ball"></span>',
			        '<span class="mui-green-ball"></span>',
			        '<span class="mui-yellow-ball"></span>',
			        '<span class="mui-red-ball"></span>',
			    '</div>'].join('');
				$dom.html(loadHtml);
				$loadDom = $dom.find('.mui-ball-loading');
			};
...
(function(){
	document.body.parentNode.style.fontSize = window.innerWidth * 100 / 320 + 'px';
	window.Rem2Px = window.innerWidth*100/320;
    document.body.style.opacity = 1;

    window.H5COM = {
		//合并参数（后者合并到前者）
		combineWith : function(o1,o2){
		    for(var i in o2){
		        o1[i] = o2[i];
		    }
		    return o1;
		},
    	checkSameWeek : function(old,now){
